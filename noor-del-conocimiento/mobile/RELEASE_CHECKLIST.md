...
- [x] API key solo server-side en producción (`ANTHROPIC_API_KEY`); camino directo de dev
      bloqueado con `__DEV__`; `.env*` en `.gitignore`.
...
- [ ] Desplegar `app/api/feedback+api.ts` y configurar `ANTHROPIC_API_KEY` como EAS Secret.
